  ...
  ({ className, dropzoneOptions, value, onValueChange, reSelect, orientation = 'vertical', children, dir, ...props }, ref) => {
    ...
    const onDrop = useCallback(
      (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
        ...
        if (!files) {
          ToastError('file error , probably too big');
          return;
        }
        ...
        if (rejectedFiles.length > 0) {
          for (let i = 0; i < rejectedFiles.length; i++) {
            if (rejectedFiles[i].errors[0]?.code === 'file-too-large') {
              ToastError(`File vượt quá dung lượng ${maxSize / 1024 / 1024}MB`);
              break;
            }

            if (rejectedFiles[i].errors[0]?.code === 'file-invalid-type') {
              ToastError(`File phải đúng định dạng ${rejectedFiles[i].errors[0].message?.split('File type must be ')[1]}`);
              break;
            }
            if (rejectedFiles[i].errors[0]?.message) {
              if (rejectedFiles[i].errors?.at(0)?.code === 'too-many-files') {
                ToastError(`Upload tối đa ${maxFiles} files`);
              } else {
                ToastError(rejectedFiles[i].errors[0].message);
              }
              break;
            }
          }
...
export const FileUploaderContent = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ children, className, ...props }, ref) => {
  ...
  return (
    <div className={cn('w-full px-1', className)} ref={containerRef} aria-description='content file holder'>
...
export const FileUploaderItem = forwardRef<HTMLDivElement, { index: number } & React.HTMLAttributes<HTMLDivElement>>(
  ({ className, index, children, ...props }, ref) => {
    ...
    return (
      ...
      <div
        ref={ref}
        className={cn(buttonVariants({ variant: 'ghost' }), 'h-6 p-1 justify-between  relative', className, isSelected ? 'bg-muted' : '')}
        {...props}>
...
export const FileInput = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, children, ...props }, ref) => {
  ...
  return (
    <div ref={ref} {...props} className={`relative w-full ${isLOF ? 'opacity-50 cursor-not-allowed ' : 'cursor-pointer'} ${className}`}>
      <div
        className={cn(
