...
const MultiSelector = ({ values: value, onValuesChange: onValueChange, loop = false, className, children, dir, ...props }: MultiSelectorProps) => {
  ...
  return (
    <MultiSelectContext.Provider
      ...
      <Command onKeyDown={handleKeyDown} className={cn('overflow-visible bg-transparent flex flex-col', className)} dir={dir} {...props}>
        {children}
      </Command>
    </MultiSelectContext.Provider>
  );
...
const MultiSelectorTrigger = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, children, ...props }, ref) => {
  ...
  return (
    <div ref={ref} className={cn('flex flex-wrap gap-1 p-1 py-2 rounded-md border border-input bg-background', className)} {...props}>
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <CommandPrimitive.Input
        ...
        className={cn(
          'rounded-md bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 leading-none tracking-tight ',
          className,
