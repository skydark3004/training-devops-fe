...
export const createExerciseSchema = z.object({
  ...
  exerciseType: z.enum(listExerciseType, { message: 'Bạn phải chọn loại bài tập' }),
  module: z.string({ message: 'Bạn phải chọn module' }).uuid({ message: 'Bạn phải chọn module' }),
  studyProgramCode: z.string({ message: 'Bạn phải chọn chương trình học' }),

  description: z.string().trim().max(255, 'Tối đa 255 kí tự'),
  thumbnail: z.any(),
  ...
  VIDEO: z.array(
    z
      ...
      .superRefine((value, ctx: RefinementCtx) => {
        if (value.typeOfScreen === EnumScreenTypeOfVideo.VIDEO && !value?.uploadVideoFile?.length) {
          ctx.addIssue({
        ...
        if (value.typeOfScreen === EnumScreenTypeOfVideo.ANSWER) {
          if (!value?.nameOfButton?.trim()?.length) {
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
          ...
          if (value?.nameOfButton?.trim()?.length > 100) {
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
        ...
        moreComponents: z.array(
          z
            ...
            .superRefine((value, ctx) => {
              if (value.type === EnumTypeComponentOfFlexibleScreen.IMAGE && !value?.uploadImage) {
                ctx.addIssue({
                  path: ['uploadImage'],
                  code: z.ZodIssueCode.custom,
...
export type TypeCreateExerciseSchema = z.infer<typeof createExerciseSchema>;
